...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($email) || empty($phone_number) || empty($address)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE Clients SET name = ?, email = ?, phone_number = ?, address = ? WHERE id = ?");
            $stmt->execute([$name, $email, $phone_number, $address, $client_id]);
    ...
    <div class="container mt-5">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" id="phone_number" name="phone_number" class="form-control" value="<?php echo htmlspecialchars($client['phone_number']); ?>" required>
            </div>
            ...
            <a href="client_dashboard.php" class="btn btn-secondary">Go Back</a>
        </form>
    </div>
